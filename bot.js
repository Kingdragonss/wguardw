...
client.on("message", async msg => {
  ...
  let a = await db.fetch(`kufur_${msg.guild.id}`)
    if (a == 'acik') {
      const küfür = [
        "amk","it","yarak","mk", "amk", "puşt", "aq","sg","orospu", "oruspu", "oç", "sikerim", "yarrak", "piç", "amq", "sik", "amcık", "çocu", "sex", "seks", "amına", "orospu çocuğu", "sg", "siktir git","31","ananın amına yarak"
                  ]
          ...
          try {
            if (!msg.member.hasPermission("MANAGE_GUILD")) {
                  msg.delete();
                    ...
                    return msg.channel.send(`<:hydrahoca:808066989439582250> **Küfür Etme!** <a:hydrahayr:816551461219205160>`).then(msg => msg.delete(1000));
            }              
                } catch(err) {
                  console.log(err);
                }
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('KICK_MEMBERS')) {
          ...
          return message.reply('**Reklam yapmaya devam edersen atılacaksın <a:hydrahayr:816551461219205160> **').then(message => message.delete(3000));
